...
contract PlanetStake is IERC1155Receiver, Stake, IPlanetStake {
    ...
    function stake(uint256 _tokenId) external override {
        ...
        // Update staking data
        stakingHistory.stakeData.stakingDate = uint64(block.timestamp); //25 jan
        stakingHistory.tokenId = _tokenId; //0 
        stakingHistory.quantity += 1; //2
    ...
    function claim(
        ...
        require(
            _lrtDistributor.distribute(bytes32("Game"), _rewardAmount, _staker),
            "PlanetStake::Fail transfer in staking claim"
        );
    }

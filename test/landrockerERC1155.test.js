...
describe("LandRockerERC1155", function () {
  ...
  it("should revert set floor price when caller is not admin", async function () {
    const tokenId = await landRockerERC1155Instance
      .connect(approvedContract)
      .callStatic.safeMint(owner.address, 10);
    ...
    await expect(
      landRockerERC1155Instance.connect(owner).setFloorPrice(tokenId, 0)
  ...
  it("should revert if floor price is equal to 0", async function () {
    ...
    await expect(
      landRockerERC1155Instance.connect(admin).setFloorPrice(tokenId, 0)
